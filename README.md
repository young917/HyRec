...
Similarly, you can compute nine properties from the hypergraphs extrapolated by HyperK and evaluate them against baselines by following these steps:
...
```
cd Property/results/
python make_input_list_ext.py --dataname {half_dataname} --inputdir {directory_path_for_trained_HyperK}
```
2. **Compute Properties.** Run the following script to compute nine properties from all hypergraphs extrapolated or generated by HyperK:
```
cd Property/run/
./run_eval_ext_HyperK_{half_dataname}.sh
./run_eval_ext_HyperK_{target_full_dataname}.sh
```
3. **Select Best Hypergraph.** Run the following command to select the best generated hypergraph:
```
cd Property/analyze_extrapolation/
python ablation_half.py --dataname {half_dataname} --ablation_target HyperK
```
4. **Prepare Competitor Results.** Compute nine properties from hypergraphs generated by competitors (HyperCL, HyperFF, HyperLAP, HyperPA, THera):
```
cd Property/run/
