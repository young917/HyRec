...
def read_properties(dataname, algoname, modelindex=-1):
    if "answer" == algoname:
        ...
        graphpath = "../dataset/" + dataname + "_ext_pa.txt"
        outputdir = "../results/{}/{}/".format(algoname, dataname)
    elif "thera" == algoname:
        ...
        graphpath = "../dataset/tr/{}_ext_tr_{}.txt".format(dataname, modelindex)
    ...
    for distname in ["clusteringcoef_hedge", "density_dist", "overlapness_dist"]:
        ...
        dist[distname] = {}
        X = []
        if os.path.isfile(outputdir + distname + "_processed.txt"):
            X, Y = [], []
            with open(outputdir + distname + "_processed.txt", "r") as f:
                for line in f.readlines():
