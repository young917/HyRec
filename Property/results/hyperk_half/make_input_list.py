...
for dataname in datalist:
    print(dataname)
    outputshfile = f"../../run/run_hyperk_half_{dataname}.sh"
    if os.path.isfile(outputshfile):
        os.remove(outputshfile)
    with open(outputshfile, "w") as f:
        f.write("cd ..\n")

    answer_dataname = dataname #fulldatalist[dataname]
    answer_hedgeset = set()
    answer_nodeset = set()
    ...
    for rootdir in ["../../../Model/result/"]:
        ...
        for dirname in os.listdir(rootdir + dataname):
            ...
            if "eval_log.txt" not in filelist:
                flag = False
            if "sampled_0.txt" not in filelist:
                flag = False
            if flag is False:
                continue
            # Valid Output!
            try:
                ...
                if os.path.isfile(curdir + "final_eval_log.txt"):
                    with open(curdir + "final_eval_log.txt", "r") as f:
                        for line in f.readlines():
                            ep_str, loss_str = line.rstrip().split(", ")
                            ep = int(ep_str.split(":")[-1])
                            lossSV_eval = float(loss_str.split(":")[-1])
                else:
                    with open(curdir + "eval_log.txt", "r") as f:
                        for line in f.readlines():
                            ep_str, loss_str = line.rstrip().split(", ")
                            ep = int(ep_str.split(":")[-1])
                            lossSV_eval = float(loss_str.split(":")[-1])
            ...
            if len(entire_hedgeset) > 0:
                ...
                with open(outputshfile, "+a") as f:
                    f.write("./bin/Sampling --inputpath {} --outputdir ./results/hyperk_half/{}/{}/\n".format(curpath, dataname, output_count))
                    f.write("cd src\n")
                    f.write("python calculation_helper.py --inputpath {} --outputdir ../results/hyperk_half/{}/{}/ --sv --effdiam\n".format(curpath, dataname, output_count))
                    f.write("cd ../\n")
