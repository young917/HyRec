...
for dataname in datalist:
    ...
    answer_svs = []
    with open(f"../answer/{dataname}/sv.txt", "r") as f:
        for line in f.readlines():
            cur_sv = float(line.rstrip())
            answer_svs.append(cur_sv)
    answer_svs = np.array(answer_svs)
    ...
    for rootdir in ["../../../Model/result/"]: 
        ...
        for dirname in os.listdir(rootdir + dataname):
            ...
            tmp = dirname.split("_")
            init_row, init_col, k = int(tmp[0]), int(tmp[1]), int(tmp[2])
